...
void init_usart1(void)
{
	...
	//GPIO Configurations
	//MODER USART_TX(PB6) pin = 10 => General purpose output mode
	...
	//OT USART1_TX(PB6) pin = 0 => Output push-pull
	CLEAR_BIT(GPIO_BASE(GPIO_PORTB) + GPIOx_OTYPER_OFFSET, OTy_BIT(6));

	//OSPEEDR USART1_TX(PB6) pin = 01 => Medium speed
	...
	//PUPDR USART1_TX(PB6) pin = 00 => No pull-up, pull-down
	...
	//MODER USART1_RX(PB7) pin = 10 => General purpose output mode
	...
	//OT USART1_RX(PB7) pin = 0 => Output push-pull
	CLEAR_BIT(GPIO_BASE(GPIO_PORTB) + GPIOx_OTYPER_OFFSET, OTy_BIT(7));

	//OSPEEDR USART1_RX(PB7) pin = 01 => Medium speed
	...
	//PUPDR USART1_RX(PB7) pin = 00 => No pull-up, pull-down
...
void usart1_handler()
{
	if(READ_BIT(USART1_BASE + USART_SR_OFFSET, ORE_BIT))
	{
		...
		usart1_send_char('@');
		char a = (char)REG(USART1_BASE + USART_DR_OFFSET)
	}
	else
	{
		char ch;
